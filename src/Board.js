...
class ForegroundTableRow extends React.Component { 
    render() { 
        ...
        for ( let column  = 0; column < 19; column++ ) {
            ...
            cells.push(<td className="cell2" key={Math.random()}  onClick={(e)=>this.props.onClick(this.props.row, column)}><div key={Math.random()}>{owner}</div></td>)
        }
        return (<tr key={Math.random()}>{cells}</tr>)
    }
}
...
class Board extends React.Component {
    ...
    render() {  
        ...
        for ( let rowIndex = 0 ; rowIndex < 19; rowIndex++ ) { 
            fgRows.push(<ForegroundTableRow data={this.props.data[rowIndex]} row={rowIndex} key={Math.random()}  onClick={this.props.onClick.bind()} />)
        }
